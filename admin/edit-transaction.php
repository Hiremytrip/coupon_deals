...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($user_id) || empty($amount) || empty($status)) {
        ...
        // Update transaction
        $sql = "UPDATE transactions SET user_id = ?, coupon_id = ?, amount = ?, status = ?, notes = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iidssi", $user_id, $coupon_id, $amount, $status, $notes, $id);
        
        if ($stmt->execute()) {
            // If status changed to approved, update user wallet balance
            ...
            // If status changed from approved to something else, deduct from wallet
...
<head>
    ...
    <title>Edit Transaction - CouponDeals Admin</title>
    <!-- Bootstrap CSS -->
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="edit-transaction.php?id=<?php echo $id; ?>" method="POST">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="user_id" name="user_id" required>
                                        ...
                                        <?php foreach ($users as $user): ?>
                                            <option value="<?php echo $user['id']; ?>" <?php echo $transaction['user_id'] == $user['id'] ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="coupon_id" name="coupon_id">
                                        ...
                                        <?php foreach ($coupons as $coupon): ?>
                                            <option value="<?php echo $coupon['id']; ?>" <?php echo $transaction['coupon_id'] == $coupon['id'] ? 'selected' : ''; ?>>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="amount" name="amount" step="0.01" value="<?php echo $transaction['amount']; ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="status" class="form-label">Status*</label>
                                    <select class="form-select" id="status" name="status" required>
                                        <option value="pending" <?php echo $transaction['status'] == 'pending' ? 'selected' : ''; ?>>Pending</option>
                                        <option value="approved" <?php echo $transaction['status'] == 'approved' ? 'selected' : ''; ?>>Approved</option>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="3"><?php echo $transaction['notes']; ?></textarea>
                            </div>
                            <div class="d-grid">
